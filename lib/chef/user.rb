...
class Chef
  class User
    ...
    def chef_rest_v1
      @chef_rest_v1 ||= get_versioned_rest_object(Chef::Config[:chef_server_url], "100")
    end
    ...
    def chef_root_rest_v1
      @chef_root_rest_v1 ||= get_versioned_rest_object(Chef::Config[:chef_server_root], "100")
    end
    ...
    def destroy
      Chef::REST.new(Chef::Config[:chef_server_url]).delete_rest("users/#{@username}")
    end

    def create
      # try v1, fail back to v0 if v1 not supported
      begin
        payload = { :username => self.username, :admin => self.admin, :password => self.password }
        payload[:public_key] = public_key if public_key
        payload[:create_key] = create_key if create_key
        raise Chef::Exceptions::InvalidUserAttribute, "You cannot set both public_key and create_key for create." if @create_key && initial_public_key
        ...
        handle_version_http_exception(e, SUPPORTED_API_VERSIONS[0], SUPPORTED_API_VERSIONS[-1])
        payload = {
